...
-> We use a dictionary (Python's dict) to store elements as keys 
and their corresponding indices as values. This allows us to 
quickly check whether a complement is present in the array.
...
-> We check if the complement exists in the num_dict by using the in 
operator. If the complement is found, we return the indices of the 
two numbers.

-> If the complement is not found, we add the current number and 
its index to the num_dict for future reference.

->If no solution is found after iterating through the entire array, 
we return an empty list, indicating that there is no valid solution.

The time complexity of this solution is O(n), where n is the 
number of elements in the input array. This algorithm is efficient for
finding two numbers that sum up to the target, and the dictionary (hash map)
data structure provides a fast lookup mechanism.
'''